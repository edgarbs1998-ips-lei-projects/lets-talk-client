    ...
    def run(self):
        while True:
            try:
                ...
                break
            if not received:
                self.master.insert_message("red", "$ Communication with the server has been terminated...")

                # Close socket
                self.master.client_socket.shutdown(socket.SHUT_RDWR)
                self.master.client_socket.close()

                break

            received_messages = received.split("$")
            for received_message in received_messages:
                if not received_message:
                    continue

                received_type_args = received_message.split(" ", 1)
                ...
                if received_type == enums.ClientAction.ACTIVE_CHANNEL.value:
                    ...
                    # Close window
                    time.sleep(settings.SECONDS_BEFORE_EXIT)
                    ...
                    self.master.insert_message("salmon4", "[PRIVATE MESSAGE] [%s] %s: %s"
